...
// Structure to sync time to NodeTroopers
...
setInterval(function() {
  fetch('/status')
    ...
    .then(data => {
      ...
      document.getElementById('radar2').innerText = data.esp2 ? (data.esp2Human ? "Human Found" : "No Human") : "Unknown";
      ...
      document.getElementById('radar3').innerText = data.esp3 ? (data.esp3Human ? "Human Found" : "No Human") : "Unknown";
...
<div class="status-box">
  <p>NodeTrooper 47: <span id="status2" style="color:red;">Checking...</span> | Radar: <span id="radar2" class="radar-badge">Unknown</span></p>
  <p>NodeTrooper 16: <span id="status3" style="color:red;">Checking...</span> | Radar: <span id="radar3" class="radar-badge">Unknown</span></p>
...
// Callback when data is received from NodeTroopers (Heartbeat & Radar data)
...
void handleSetTime() {
  if (server.hasArg("h") && server.hasArg("m")) {
    ...
    // Broadcast the exact time to all Workes via ESP-NOW
    ...
    Serial.println(">>> Time broadcasted to Workes!");
