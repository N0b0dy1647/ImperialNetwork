...
bool autoTurnOnEnabled = true; // NEW: Controls if radar is allowed to turn light ON automatically
...
// --- Time Check Function for the Slave ---
bool isNight() {
  if (!isTimeSet) return false; // Default to day if no time is known
  struct tm timeinfo;
...
void OnDataRecv(const esp_now_recv_info *info, const uint8_t *data, int len) {
  ...
  // 1. If it is a TIME SYNC message from the Master
  if (baseMsg.msgType == MSG_TYPE_TIME && len == sizeof(time_message)) {
    ...
    MONITOR_SERIAL.printf(">>> Slave Time Synced: %02d:%02d:%02d\n", timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
  }
  ...
  // 2. If it is a COMMAND message
    ...
    if (cmdMsg.target == MY_ID || cmdMsg.target == 0) {
      ...
      IrSender.sendNEC(IR_ADDRESS, cmdMsg.cmd, 0);

      // --- SMART MANUAL OVERRIDE ---
      if (cmdMsg.cmd == CMD_OFF) {
        isLightOn = false;
        autoTurnOnEnabled = false; // User wants darkness. Disable Auto-ON algorithm!
        MONITOR_SERIAL.println("[MODE] Manual OFF received. Auto-ON paused.");
      } else {
        isLightOn = true;
        autoTurnOnEnabled = true;  // User turned light ON (or changed color). Re-enable Auto-ON algorithm!
        MONITOR_SERIAL.println("[MODE] Manual ON received. Auto-ON active.");
      }
    }
  }
}

// Function to broadcast the heartbeat with radar status to the master
...
void setup(void) {
  ...
  // --- Initialize Radar (From your exact logic) ---
...
void loop(void) {
  ...
  if (radar.read()) {
    ...
    // Detect state changes (Target lost or found)
    ...
    // --- AUTOMATION LOGIC ---
    // Auto-ON: Human < 2m, it is night, light is off AND Auto-ON is allowed!
    ...
    // Auto-OFF: Human completely leaves the room, and light is on
      ...
      isLightOn = false;
      // We do NOT change autoTurnOnEnabled here. 
      // If the radar turned it off, it is allowed to turn it back on next time you enter!
    }
  }
